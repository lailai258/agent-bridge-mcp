...
`agent-bridge-mcp` 是一个只暴露 MCP server 能力的 Node.js 项目。它不直接调用模型 API，而是通过本机已安装并登录的 Claude、Codex、Forge、OpenCode、Antigravity CLI 启动后台任务，并用 MCP tools 提供统一的运行、查询、等待、短窗口观察、终止、清理、诊断和模型发现能力。
...
- 通过 `run` 启动 Claude、Codex、Forge、OpenCode、Antigravity CLI 后台任务，并立即返回 PID。
...
- Gemini CLI 已移除；旧的 `gemini-*` 和 `gemini-ultra` 输入会返回参数错误。
...
- 本地按需安装并登录 Claude、Codex、Forge、OpenCode、Antigravity CLI。
- 使用 npm 安装依赖。
