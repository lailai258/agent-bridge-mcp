...
`agent-bridge-mcp` 是 MCP-only 项目，核心职责是在 MCP server 内通过本机 AI CLI 启动和管理后台任务。项目不直接调用模型 API，不提供人类 CLI 子命令，也不持久化进程状态。
...
- 在内存 `Map<pid, TrackedProcess>` 中记录状态。
- 收集 stdout/stderr。
...
`ProcessService` 使用内存 `Map` 保存进程句柄、状态、输出和元数据。后续 tools 通过 PID 操作同一条内存记录。
...
项目没有数据库、ORM 或迁移脚本。进程状态只存在当前 Node.js 进程内存中：

- server 重启后不会恢复旧 PID。
- `cleanup_processes` 会移除已完成或失败的内存记录。
- `get_result` 和 `wait` 只能访问当前 server 生命周期内的 PID。

这属于刻意设计，避免把本项目重新扩展为带状态目录的人类 CLI 系统。
