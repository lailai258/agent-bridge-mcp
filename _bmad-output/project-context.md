...
- 外部 CLI 行为必须用 mock CLI 脚本或 mocked `spawn` 覆盖，不要在单元测试中依赖本机真实 Claude/Codex/Gemini/Forge/OpenCode 登录态。
...
- 不要给 Gemini、Forge、OpenCode 传 `reasoning_effort`；这些 agent 当前不支持该参数。
